...
<div class="pull-right mb-2">
    <a id="add-company" class="btn btn-success" href="{{ url('create-clients-company') }}"> <i class="fas fa-plus"></i>  اضافة شركة عميل  </a>
    </div>
  
<h2> بيانات شركات العملاء   </h2>
</div>
...
<th class="col-md-2">اسم البنك</th>
<th class="col-md-4"> رقم الايبان IBAN</th>

<th width="80px">تعديل</th>

</tr>
@foreach($clientcompany as $item)
<tr>
<th> {{$item->company_code}} </th>
<th> {{$item->company_name }}   </th>
<th>{{$item->Commercial_record_number}}  </th>
<th>{{$item->tax_number}} </th>
<th>{{$item->bank_name }}  </th>
<th>{{$item->bank_number}} </th>
<th> {{$item->password }} </th>
<th> {{$item->password_confirmation }} </th>
<th>
  <img src="{{ asset('uploads/clientsCompanies/'.$item->iban_iamge)}}" width="70px" height="70px" alt="image">
  </th>
